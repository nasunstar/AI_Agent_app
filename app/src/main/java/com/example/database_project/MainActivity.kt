                ...
                Button(onClick = {
                    lifecycleScope.launch(Dispatchers.IO) {
                        try {
                            ...
                            val encodedBody =
                                "7JWI64WV7ZWY7IS47JqUIO2UhOuhnOygne2KuCDtmozsnZjrpbwg7KeE7ZaJ7ZWY6rOgIOyLtuyKteuLiOuLpCDsnbTrjIDrpqwg65Oc66a8Lg==\n"
                            val body = String(
                            ...
                            App.db.withTransaction {
                                ...
                                val typeId =
                                    App.db.eventTypeDao().getByName(parsed.category)?.id
                                        ?: App.db.eventTypeDao().insert(
                                            EventType(
                                                typeName = parsed.category,
                                                subdivision = null
                                            )
                                        )

                                val contactId =
                                    App.db.contactDao().getByName(parsed.sender)?.id
                                        ?: App.db.contactDao().insert(
                                            Contact(
                                                name = parsed.sender,
                                                email = fromEmail,
                                                phoneNumber = null
